"""Run the xorsat26 26-variable solver with one shared LFSR on PYNQ-Z1."""
import argparse
import hashlib
import json
import os
...
def check():
    ...
    require(len(problem['clauses']) in [72, 112], 'Invalid clause count')
    require(all(len(c) == 3 and len(set(map(abs, c))) == 3 and all(1 <= abs(v) <= 26 for v in c) for c in problem['clauses']), 'Invalid clauses')
    return cfg, problem
...
def load():
    ...
    require(io.read(0) == 0x584f3236 and io.read(4) == 0x30000, 'Wrong shared-LFSR hardware identity')
...
def main():
    ...
    cfg, problem = check()
    if args.check:
        print('PASS: deployment files and fixed xorsat26 problem verified')
        return
        ...
        with (out / 'trials.jsonl').open('x') as log:
            for trial in range(args.trials):
                ...
                for policy in modes:
                    ...
                    for name, offset in [('initial_bits', 0x1c), ('final_bits', 0x20), ('final_energy', 0x28), ('updates', 0x34), ('rng_words', 0x38), ('rng_steps', 0x3c), ('cycles', 0x40), ('first_hit', 0x44), ('rng_state', 0x48), ('sweeps', 0x4c), ('reuse', 0x68), ('fresh', 0x6c), ('refills', 0x70), ('initial_draws', 0x74)]:
                        row[name] = io.read(offset)
                    for name in ['final_energy']:
                        row[name] = signed(row[name])
                    # Native probSAT stops at a solution or the 26*1024 flip cap.
                    require(row['updates'] <= 26624 and row['fresh'] + row['reuse'] == row['updates'], 'Flip counts')
                    require(row['initial_draws'] == 26 and row['rng_words'] == 26 + row['fresh'] + row['refills'], 'Source counts')
                    require(row['rng_steps'] == 32 * row['rng_words'], 'LFSR step count')
                    if policy == 0:
                        require(row['reuse'] == 0 and row['fresh'] == row['updates'], 'IID counts')
                    unsat = sum(not any(((row['final_bits'] >> (abs(v)-1)) & 1) == (v > 0) for v in c) for c in problem['clauses'])
                    require(row['final_energy'] == unsat, 'Independent CNF score')
                    require(unsat == 0 or row['updates'] == 26624, 'Premature stop')
                    if 'native_problem' in problem:
                        violated = sum((sum((row['final_bits'] >> i) & 1 for i in c['variables']) % 2) != c['parity'] for c in problem['native_problem']['native_clauses'])
                        require(violated == unsat, 'Independent XOR score')
                    row['success'] = unsat == 0
