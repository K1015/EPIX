...
def validate(path):
    ...
    def wired(name, signal, source, source_port, polarity=None):
        p = port(name, signal)
        targets = {(c.get("INSTANCE"), c.get("PORT"))
                   for c in p.findall("./CONNECTIONS/CONNECTION")}
        require((source, source_port) in targets,
                "connection: {}/{} <- {}/{}".format(name, signal, source, source_port))
        if polarity:
            require(p.get("POLARITY") == polarity,
    ...
    for name, signal in ((reset, "slowest_sync_clk"),
    ...
    require(fclk.get("CLKFREQUENCY") == "100000000", "FCLK0 metadata frequency is 100 MHz")
    require(port("ps7_0", "FCLK_RESET0_N").get("POLARITY") == "ACTIVE_LOW",
            "PS7 reset output is active low")
    return {"schema": "maxcut26_hwh_reset_validation_v1", "status": "PASS",
            "hwh_sha256": hashlib.sha256(raw).hexdigest(), "checks": checks,
