`timescale 1ns/1ps
// Fixed maxcut_n26_i01. All arithmetic, annealing, source requests and scoring
// run in PL. One p-bit is updated at a time, in node order 0..25.
module maxcut26_core #(
    ...
    output reg [25:0] spins, best_spins, initial_spins,
    ...
    localparam signed [15:0] TARGET_ENERGY = -16'sd80;
    reg [3:0] phase;
    reg [4:0] node_index, column_index, energy_row;
    reg [9:0] sweep_index;
    ...
    reg [31:0] cached_word [0:25];
    reg signed [7:0] cached_q [0:25];
    reg [1:0] cache_age [0:25];
    reg [25:0] cache_valid;
    integer cache_index;
    ...
    (* rom_style = "block" *) reg signed [3:0] weights [0:675];
    ...
    wire [4:0] weight_row = (phase == E_REQ || phase == E_ACC) ? energy_row : node_index;
    wire [9:0] weight_addr = {5'b0, weight_row} * 10'd26 + {5'b0, column_index};
    ...
    // This Ising family reports energy; the Max-Cut-only register is unused.
    assign best_cut = 0;
    ...
    always @(posedge clk) begin
        if (!rst_n) begin
            ...
            for (cache_index=0; cache_index<26; cache_index=cache_index+1) begin
            ...
            trace_valid <= 0; trace_take_fresh <= 0; trace_age_after <= 0; trace_visit <= 0; trace_word <= 0; trace_best_visit <= 0;
            ...
            if (abort_run && busy) begin
                ...
                INIT_WAIT: if (source_valid) begin
                    ...
                    if (node_index == 25) phase <= INIT_END;
                ...
                E_ACC: begin
                    ...
                    if (column_index == 25) begin
                        column_index <= 0;
                        if (energy_row == 25) phase <= E_DONE;
                ...
                F_REQ: phase <= F_ACC;
                F_ACC: begin
                    if (column_index == 25) begin field <= field_sum; phase <= Q_REQ; end
