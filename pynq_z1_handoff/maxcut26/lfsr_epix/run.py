...
def check():
    ...
    require(problem['pbits'] == 26 and problem['target_cut'] == 285 and len(edges) == 131 and sum(w for i, j, w in edges) == 398, 'Problem does not match this fixed FPGA image')
    require(all(0 <= i < j < 26 and isinstance(w, int) and w > 0 for i, j, w in edges), 'Invalid graph edges')
    require(len(problem['trial_seeds']) == 1000 and all(isinstance(x, int) and 0 < x < 2**32 for x in problem['trial_seeds']), 'Invalid LFSR trial seeds')
    return cfg, problem
...
def load():
    ...
    require(io.read(0) == 0x4d433236 and io.read(4) == 0x20001, 'Wrong shared-LFSR hardware identity')
...
def main():
    ...
    try:
        ...
        with (out / 'trials.jsonl').open('x') as log:
            for trial in range(args.trials):
                ...
                for policy in modes:
                    ...
                    # 26 initialization words, then one word per fresh update.
                    # EPIX reuses cached words; all runs complete 26 * 1024 updates.
                    ...
                    # Recompute scores from the returned spins: E = total weight - 2*cut.
                    for state, energy in [('final_bits', 'final_energy'), ('best_bits', 'best_energy')]:
                        cut = sum(w for i, j, w in problem['edges'] if ((row[state] >> i) ^ (row[state] >> j)) & 1)
                        require(row[energy] == 398 - 2 * cut, 'Independent graph score')
                    require(row['best_cut'] == (398 - row['best_energy']) // 2 and row['best_cut'] <= 285, 'Best cut mismatch')
                    # Each word takes 32 LFSR steps; reuse also skips its request cycle.
        ...
        for policy in sorted(set(r['mode'] for r in rows)):
            ...
            result = {'trials': len(group), 'successes': sum(r['best_cut'] == 285 for r in group)}
            for key in ['rng_words', 'rng_steps', 'core_seconds', 'host_seconds', 'reuse']:
