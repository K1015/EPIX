...
int main(int argc,char** argv) {
    try {
        ...
        for(int i=0;i<MEMORY_WORDS;++i) {
            memory>>std::hex>>coefficient;require(!memory.fail(),"truncated coefficient image");coefficient_image.push_back(coefficient);
        ...
        while(!d.done) {
            ...
            if(CACHE_XOR && int(d.debug_phase)==3)for(int i=0;i<SITES;++i)require(bool(extract(d.debug_dirty,i,1))==cache.dirty[i],"dirty-field state mismatch");
            ...
            if(take) {
                ...
                bool expected_cached=!rf&&valid[site]&&fields[site]==q;
                require(bool(d.event_word_valid)==!expected_cached,where+": cached-sample interface");
                rng[site]=word;
        ...
        if(CACHE_XOR){expected_reads=cache.reads;require(run_cycles-stalled==cache.cycles,"field-cache active-cycle count");}
