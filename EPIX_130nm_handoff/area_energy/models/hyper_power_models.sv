...
module sram22_128x40m4w20(input wire clk,rstb,ce,we,
 ...
 import "DPI-C" function void a1_macro_init(input string name);
 import "DPI-C" function void a1_macro_edge(input string name,input int rising,input int ce_value,input int we_value);
 initial a1_macro_init($sformatf("%m"));
 always @(posedge clk) a1_macro_edge($sformatf("%m"),1,int'(ce),int'(we));
 always @(negedge clk) a1_macro_edge($sformatf("%m"),0,int'(ce),int'(we));
 reg [39:0] mem [0:127];
