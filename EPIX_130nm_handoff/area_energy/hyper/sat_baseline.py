"""Measure the baseline with EPIX disabled and unused logic removed."""
...
'''
    ...
    metadata_bits = {
        b
        for n, v in module['netnames'].items()
        if re.search(r'(saved_q|valid_q|ages_q|history_q|history_flat)(\[.*\])?$', n)
        for b in v['bits']
        if isinstance(b, int)
    }
    metadata_ff = {
        owners[b][0]
        for b in metadata_bits
        if b in owners and 'df' in owners[b][1]['type']
    }
    assert not metadata_ff, (
        'EPIX metadata survived IID-only mapping',
        sorted(metadata_ff)[:10],
    )
    assert all(
    ...
    save(
        ...
        dict(
            ...
            epix_metadata_ff_bits=len(metadata_ff),
