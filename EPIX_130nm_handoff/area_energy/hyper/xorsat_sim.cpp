...
int main(int argc,char** argv){try{
    ...
    std::vector<uint64_t> coefficient_image;
    uint64_t setup_begin=cycles;snapshot(".warm.saif");a1_set_window(1);d.rst=0;
    for(int i=0;i<MEMORY_WORDS;++i){uint64_t value;memory>>std::hex>>value;require(!memory.fail(),"short SRAM image");coefficient_image.push_back(value);d.coeff_we=1;d.coeff_address=i;d.coeff_data=value;tick();require(!d.error_code,"coefficient load");}
    d.coeff_we=0;
    ...
    CacheReference cache(coefficient_image,SITES,expected_visits,FACTOR_COUNT);
    std::vector<int> cached_fields(SITES,0),cached_bits(SITES,0),cached_valid(SITES,0);
    while(!d.done){
        ...
        if(take){
            ...
            if(CACHE_XOR)cache.commit(visit,site,q,old,next,rf);
            ...
            bool expected_cached=!rf&&cached_valid[site]&&cached_fields[site]==q;
            require(bool(d.event_word_valid)==!expected_cached,"cached-sample validity"+at);
            if(expected_cached)require(next==cached_bits[site],"cached sample"+at);
            else require(word==d.event_word,"sampled RNG word"+at);
            require(beta==d.event_beta&&rf==d.event_fresh,"RNG/policy"+at);
            if(rf){cached_fields[site]=q;cached_bits[site]=next;cached_valid[site]=1;}
            if(clear)std::fill(cached_valid.begin(),cached_valid.end(),0);
    ...
    if(CACHE_XOR){expected_reads=cache.reads;if(!stalls)require(end-search_begin==cache.cycles,"field-cache cycle count");}
