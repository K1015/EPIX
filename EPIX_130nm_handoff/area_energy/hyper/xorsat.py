"""Build, map, and measure the XOR-SAT solver in SKY130."""
...
def sources(case):
    return [
        case / 'top.sv',
        A2 / 'variants/xor/rtl/a2_core.sv',
        A2 / 'rtl/a2_icg.sv',
...
def build(case, out, mapped=False):
    ...
    cmd = [
        ...
        '2000',
        '-CFLAGS',
        '-O1 -std=c++17 -I' + str(case) + ' -I' + str(ROOT / 'flow') + ' -DCACHE_XOR=1',
    ]
    if mapped:
...
'''
    ...
    def stored_bits(suffix):
        return {
            owners[b][0]
            for n, v in module['netnames'].items()
            if n.endswith(suffix) and '.private_rng[' in n
            for b in v['bits']
            if b in owners and 'df' in owners[b][1]['type']
        }

    sample_bits = len(stored_bits('.sample_q'))
    saved_bits = len(stored_bits('.saved_q'))
    assert sample_bits == (
        0 if cfg['parameters']['FAMILY_XOR'] else cfg['n']
    ), sample_bits

    def named_ff(predicate):
        return {
            owners[b][0]
            for n, v in module['netnames'].items()
            if predicate(n)
    ...
    dirty_bits = len(named_ff(lambda n: n == 'core.dirty_flat'))
    history_bits = len(named_ff(lambda n: n == 'core.history_flat'))
    assert dirty_bits == cfg['n'], dirty_bits
    assert history_bits == 2 * cfg['n'], history_bits
    expected_saved = cfg['n'] * (cfg['parameters']['FIELD_BOUND'].bit_length() + 1)
    assert saved_bits == expected_saved, (saved_bits, expected_saved)
    ...
    save(
        ...
        dict(
            ...
            saved_field_ff_bits=saved_bits,
            cached_sample_ff_bits=sample_bits,
            dirty_field_ff_bits=dirty_bits,
            epix_history_ff_bits=history_bits,
            epix_input_tied_low=False,
...
def main():
    ...
    p.add_argument('--cases', nargs='+', default=['xorsat288'])
    p.add_argument('--stage', choices=['rtl', 'map', 'all'], default='all')
    p.add_argument('--workers', type=int, default=2)
    a = p.parse_args()
    assert all(re.fullmatch(r'xorsat(128|288|512|800)', n) for n in a.cases)
    if a.stage == 'all':
        prepare()
