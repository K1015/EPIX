...
def task(case, mode, prepare_state=False, no_lut_register_merge=False):
    ...
    if previous.exists() and json.loads(previous.read_text()).get('passed'):
        assert (
            bool(json.loads(previous.read_text()).get('no_lut_register_merge', False))
            == no_lut_register_merge
        ), 'Existing mapping uses different synthesis options; preserve it before replacement'
    ...
    result = dict(
        ...
        temperature_c=25,
        period_ns=200,
        physical_scope='pre_layout_no_wire_parasitics',
        no_lut_register_merge=no_lut_register_merge,
    )
    try:
        rt = ROOT / 'results' / case.name / mode / 'rtl/status.json'
        deadline = time.time() + 21600
        while not rt.exists() or not (
            json.loads(rt.read_text()).get('passed')
            or json.loads(rt.read_text()).get('smoke_passed')
        ):
        ...
        assert not prepare_state, 'A2 already generates scalar spin registers'
...
read_slang --no-implicit-memories --allow-use-before-declare --top {top} -I{case} -DEPIX_MODE={int(mode=='epix')} {solver_input} {case/'clusters.sv'} {ROOT/'rtl/memory.sv'} {ROOT/'rtl/aux_rng.sv'}
hierarchy -check -top {top}
synth -top {top} -flatten -noabc {'-nordff' if no_lut_register_merge else ''}
...
'''
        ...
        reusable = (
            ...
            and bool(
                json.loads(old_status.read_text()).get('no_lut_register_merge', False)
            )
            == no_lut_register_merge
        )
        if not reusable:
            ys(script, out, 'synth')
        # Reject a constant selector. Check all entries when registers stay separate.
        # Selectors changed by retiming are checked by simulation.
        # Keep RTLIL so this check can be repeated; discard the large temporary JSON.
        selection_json = out / 'selection_source.json'
        ys(
        ...
        selection = (
            check_selection.verify
            if no_lut_register_merge
            else check_selection.verify_nonconstant
        )(selection_json, case)
        selection['registers_il_sha256'] = sha(out / 'registers.il')
        (out / 'SELECTION_VALIDATION.json').write_text(
            json.dumps(selection, indent=2) + '\n'
        )
        result['selection_validation'] = selection
        ...
        expected_macros = cfg['expected_sram_macros']
        assert counts[MACRO] == expected_macros, (counts[MACRO], expected_macros)
        nets = [
            (k, v['bits'])
            for k, v in module['netnames'].items()
            if re.search(
                r'service_\d+\.replica_bank\[\d+\]\.site_bank\[\d+\]\.state$', k
            )
        ]
        assert len(nets) == cfg['rng_states'], (len(nets), cfg['rng_states'])
        aux = [
            (k, v['bits'])
            for k, v in module['netnames'].items()
            if k in ('swap_rng.state', 'cluster_rng.state')
        ]
        assert len(aux) == cfg['auxiliary_rng_states']
        bits = [x for k, b in nets + aux for x in b]
        assert (
            len(bits)
            == len(set(bits))
            == 32 * (cfg['rng_states'] + cfg['auxiliary_rng_states'])
        )
        drivers = {
            b: (name, c['type'])
        ...
        def clock_gate(clock_bit):
            seen = set()
            while clock_bit in drivers:
                assert clock_bit not in seen, 'Clock-driver cycle'
                seen.add(clock_bit)
                name, typ = drivers[clock_bit]
                cell = cells[name]
                if typ == ICG['cell']:
                    return name
                assert 'buf_' in typ or 'dlygate' in typ, (name, typ)
                clock_bit = cell['connections']['A'][0]
            raise AssertionError('RNG clock has no integrated gate')

        gate_audit = {}
        for name, state_bits in nets + aux:
            gates = {
                clock_gate(cells[drivers[bit][0]]['connections']['CLK'][0])
                for bit in state_bits
            }
            assert len(gates) == 1, (name, gates)
            gate_audit[name] = next(iter(gates))
        assert len(set(gate_audit.values())) == len(nets) + len(
            aux
        ), 'Distinct RNG banks share a gate'
        (out / 'rng_clock_audit.json').write_text(
...
if __name__ == '__main__':
    ...
    p.add_argument('--no-lut-register-merge', action='store_true')
    a = p.parse_args()
    cases = [
        x
    ...
    with cf.ThreadPoolExecutor(
        ...
        results = list(
            pool.map(
                lambda x: task(
                    *x,
                    prepare_state=a.prepare_state,
                    no_lut_register_merge=a.no_lut_register_merge,
                ),
