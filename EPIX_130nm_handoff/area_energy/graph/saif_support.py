...
def write_if_changed(path, text):
    # A running compiler may be reading these files.
    # Leave unchanged files in place.
...
'''
    ...
    original = (ROOT / 'flow/sim.cpp').read_text()
    harness = original
    substitutions = [
        (
            '#if VM_TRACE_FST',
            '#if VM_TRACE_SAIF\n#include "verilated_saif_c.h"\nusing Trace=VerilatedSaifC;\n#elif VM_TRACE_FST',
        ),
        (
            '   auto setup=cycles;',
            '   auto setup=cycles;\n   if(argc>5)trace->spTrace()->snapshot((std::string(argv[5])+".warm.saif").c_str(),ctx->time());',
        ),
        (
            '   a1_set_window(0);',
            '   if(argc>5)trace->spTrace()->snapshot((std::string(argv[5])+".setup.saif").c_str(),ctx->time());\n   a1_set_window(0);',
        ),
        (
            ' if(argc>5){eval();trace->close();}',
            ' if(argc>5){trace->spTrace()->snapshot((std::string(argv[5])+".end.saif").c_str(),ctx->time());eval();trace->close();}',
        ),
    ]
    ...
    hp = ROOT / 'flow/sim_saif.cpp'
    write_if_changed(hp, harness)
    receipt = dict(
        passed=True,
        canonical_harness_sha256=sha(ROOT / 'flow/sim.cpp'),
        observer_harness_sha256=sha(hp),
        substitutions_reversible=True,
        runtime_files={
            p.name: sha(p)
