#!/usr/bin/env python3
"""Test power activity with ns and ps library units."""
import argparse
import hashlib
import json
import os
...
def targets_and_counts():
    ...
    for kind, values in (("pin", PIN_VALUES), ("port", "d .01 .4 en .005 .6".split())):
        for name, density, duty in zip(values[::3], values[1::3], values[2::3]):
            counts[len(targets)] = [
                round(float(density) * 1000),
                round(float(duty) * 1000000),
                0,
            ] * 2
...
def check_unit(sta, source_lib, folder, unit, label):
    """Check activity values and reject incorrect annotations."""
    folder.mkdir()
    liberty = folder / "test.lib"
    assert source_lib.count('time_unit : "1ns";') == 1
    liberty.write_text(
        source_lib.replace('time_unit : "1ns";', f'time_unit : "{label}";')
    )
    (folder / "helper.tcl").write_text(activity.READBACK_TCL)
    targets, counts = targets_and_counts()
    expected = {
        window: activity.write_window(
            folder, window, targets, counts, index, 1000000, 100, unit
        )
        for index, window in enumerate(("search", "with_setup"))
    }
    script = [
        f"read_liberty {{{liberty}}}",
        f"read_verilog {{{HERE / 'pin_activity.v'}}}",
        "link_design pin_activity",
        f"create_clock -name clk -period {100 / unit:g} [get_ports clk]",
        f"source {{{folder / 'helper.tcl'}}}",
    ]
    for window in expected:
        script += [
            "sta::clear_power",
            f"source {{{folder / (window + '.activity.tcl')}}}",
        ]
        for phase in ("total", "rng"):
            script += [
                "report_power",
                f"set epix_readback_phase {phase}",
    ...
    negatives = {
        "missing_phase": "\n".join(
            line
            for line in lines
            if not line.startswith("ACTIVITY_READBACK\twith_setup\trng\t")
        ),
        "incomplete_count": result.stdout.replace(
            "\tsearch\ttotal\t15\t13\t2\t0", "\tsearch\ttotal\t14\t12\t2\t0"
        ),
    ...
    # Confirm that a wrong duty cycle fails the Tcl check.
    bad_script = script.copy()
    bad_script.insert(
        bad_script.index("report_power"),
    ...
    return dict(
        liberty_time_unit=label,
        passed=True,
        annotation_checks=60,
...
def main():
    ...
    parser.add_argument("--output", type=Path, default=Path("READBACK_VALIDATION.json"))
    args = parser.parse_args()
    sta = shutil.which(args.sta)
    if not sta:
        raise FileNotFoundError("OpenSTA unavailable; use --sta or OPENSTA")
    liberty = ROOT.parent / "tech/sky130_hd.lib"
    source_lib = liberty.read_text()
    with tempfile.TemporaryDirectory(prefix="maxcut-readback-") as directory:
        results = [
            check_unit(sta, source_lib, Path(directory) / label, unit, label)
            for unit, label in ((1.0, "1ns"), (0.001, "1ps"))
        ]
    receipt = dict(
        passed=True,
        cases=results,
        sta_binary_sha256=sha(Path(sta)),
        liberty_sha256=sha(liberty),
        fixture_sha256=sha(HERE / "pin_activity.v"),
        source_sha256={
            str(path.relative_to(ROOT)): sha(path)
            for path in (HERE / "activity.py", HERE / "test_readback.py")
        },
