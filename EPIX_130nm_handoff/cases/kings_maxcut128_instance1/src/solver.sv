...
wire [127:0] dirty_current=dirty[replica];
...
a2_aux_rng swap_rng(.clk(clk),.rst(rst),.test_enable(test_enable),.seeded(seed_we&&phase==IDLE&&seed_addr==3072),.consume(phase==SWAP),.seed_data(data_in),.word_out(swap_word));
a2_aux_rng cluster_rng(.clk(clk),.rst(rst),.test_enable(test_enable),.seeded(seed_we&&phase==IDLE&&seed_addr==3073),.consume(phase==ICM),.seed_data(data_in),.word_out(cluster_word));
...
wire [127:0] dirty[0:23];
wire [127:0] swap_spins_a=spins[swap_a],swap_spins_b=spins[swap_b];
