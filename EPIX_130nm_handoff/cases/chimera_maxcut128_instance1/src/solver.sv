...
`define EPIX_MODE 1
`endif
// 128-bit colored Gibbs solver: search control, decisions, and scores.
...
wire [0:0] cluster_ready,cluster_configured;
wire [15:0] cluster_fresh[0:0];
wire signed [15:0] cluster_score[0:0];
wire [127:0] update_bits;
...
always_comb begin
 ...
 for(integer c=0;c<1;c=c+1)begin fresh_count=fresh_count+cluster_fresh[c];score_sum=score_sum+cluster_score[c];end
end
wire [127:0] current=state_bits,dirty_current=dirty_bits;
...
wire [127:0] state_bits,dirty_bits;
    ...
    always_ff @(posedge main_control_clock) begin
        if(rst) begin
            ...
            INITIAL: if(score_commit) begin
                ...
                if(current_cut==66) begin success<=1;first_exact_sweep<=0;witness<=state_bits;end
                phase<=UPDATE;
            end
            UPDATE: if(commit) begin
                local_words<=local_words+fresh_count;
                node_visits<=node_visits+32'd64;
            ...
            SCORE: if(score_commit) begin
                ...
                if(current_cut==66 && !success) begin success<=1;first_exact_sweep<={16'b0,sweeps}+32'd1;witness<=state_bits;end
                if(!color) begin color<=1;phase<=UPDATE;end
                else begin
                    color<=0;sweeps<=sweeps+1;
                    if(sweep_in_beta==10'(SWEEPS_PER_BETA-1)) begin
                        sweep_in_beta<=0;
                        if(beta_step==4'(BETA_COUNT)) begin final_success<=(current_cut==66);phase<=FINISH;end
