`timescale 1ns / 1ps
// Shared pseudo-random source. seed_set serially shifts LFSR_in while
// pbit_enable controls when the state advances. Baseline advances every visit;
// EPIX advances only when its reuse policy asks for a fresh random word.
module rng_lfsr (clk, seed_set, pbit_enable, LFSR_in, LFSR_out);
  parameter RNG_bit_width= 32; 
  parameter [RNG_bit_width-1:0] seed = 32'b10101011110011000110111011111110;
...
// initial r_LFSR = seed;
 ...
 always_comb
    ...
    always_ff @(posedge clk) begin
      if (pbit_enable) begin
        if (seed_set) begin
          r_LFSR[1] <= LFSR_in;
          for (int i = 2; i < RNG_bit_width+1; i += 1) begin
            r_LFSR[i] <= r_LFSR[i-1];
          end
         end
         else begin
            //  r_LFSR <= {r_LFSR[RNG_bit_width:1], feedback};
            r_LFSR <= {r_LFSR[RNG_bit_width-1:1], feedback}; // New
         end
      end  
    end
