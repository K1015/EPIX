...
def clause_tables(problem, break_bits):
    ...
    require(isinstance(clauses, list) and 0 < len(clauses) <= 65535,
            'Supply clauses or cnf_clauses as a list of ternary CNF clauses.')
    for clause in clauses:
        require(len(clause) == 3 and all(type(v) is int and 0 < abs(v) <= n for v in clause)
    ...
    require(bound < 2**break_bits, 'Increase --break-bits and the matching RTL address width.')
    packed_masses = []
    for breaks in itertools.product(range(2**break_bits), repeat=3):
        a, b, c = masses(breaks)
        require(min(a, b, c) > 0 and a + b + c == 65536, 'Use positive masses totaling 65536.')
        packed_masses.append(a | (b << 16))
    lines = [f'localparam integer CLAUSES={len(clauses)};',
             'function automatic [23:0] clause_rom(input logic [15:0] index);',
             '    begin case(index)']
    for index, clause in enumerate(clauses):
        packed = sum(((abs(v) - 1) | (128 if v > 0 else 0)) << (8 * j)
    ...
    return {'masses.hex': hex_text(packed_masses, 32), 'clauses.svh': '\n'.join(lines) + '\n'}, {
        'pbits': n, 'clauses': len(clauses), 'break_address_bits': break_bits,
        'maximum_candidate_break': bound, 'mass_entries': len(packed_masses)}


def generate(family, source, output, break_bits=4, lut_path=None):
    problem = read_problem(Path(source))
    if family in ('maxcut', 'chimera'):
        lut_path = Path(lut_path) if lut_path else Path(__file__).with_name('lut32.json')
...
def main():
    ...
    parser.add_argument('--family', required=True, choices=('maxcut', 'chimera', 'sat', 'xorsat'))
    parser.add_argument('--input', required=True, type=Path)
    parser.add_argument('--output', required=True, type=Path)
    parser.add_argument('--break-bits', type=int, default=4, choices=(4, 5))
    parser.add_argument('--lut', type=Path, help='Optional 128-entry software threshold JSON.')
    args = parser.parse_args()
    try:
        result = generate(args.family, args.input, args.output, args.break_bits, args.lut)
    except (ValueError, OSError) as error:
        parser.error(str(error))
    print(f"Generated {args.family} tables for {result['pbits']} variables in {args.output}")
