...
def check():
    ...
    require(
        len(problem['J']) == 26 and problem['target_energy'] == -80,
        'Invalid Ising problem',
    )
    require(
        all(len(row) == 26 and row[i] == 0 for i, row in enumerate(problem['J'])),
        'Invalid coupling matrix',
    )
    require(
        all(
            problem['J'][i][j] == problem['J'][j][i]
            for i in range(26)
            for j in range(26)
        ),
...
def load():
    ...
    require(
        io.read(0) == 0x43483236 and io.read(4) == 0x30000,
