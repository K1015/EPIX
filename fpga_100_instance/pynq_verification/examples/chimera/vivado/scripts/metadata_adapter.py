...
def extract(hwh):
    ...
    part = (
        "xc" + system.get("DEVICE") + system.get("PACKAGE") + system.get("SPEEDGRADE")
    )
    ...
    require(
        ps.get("VLNV") == "xilinx.com:ip:processing_system7:5.5", "Unexpected PS7 IP"
    )
    require(
        accelerator.get("VLNV") == "date2027.local:user:maxcut26_axi:1.0",
        "Unexpected solver IP",
    )
    ...
    require(
        ports["M_AXI_GP0_ACLK"].get("SIGNAME") == ports["FCLK_CLK0"].get("SIGNAME"),
        "GP0 clock is not FCLK0",
    )
    axi_clock = next(
        p for p in accelerator.iter("PORT") if p.get("NAME") == "s_axi_aclk"
    )
    require(axi_clock.get("CLKFREQUENCY") == "100000000", "Solver clock mismatch")
    require(
        axi_clock.get("SIGNAME") == ports["FCLK_CLK0"].get("SIGNAME"),
        "Solver clock net mismatch",
    )
    ...
    require(
        all(address.get(k) == v for k, v in expected_map.items()),
        "Unexpected GP0 memory map",
    )
    ...
    return {
        "part": part,
        "system": system.attrib,
        "vivado_version": tree.get("VIVADOVERSION"),
        "ps_instance": ps.get("INSTANCE"),
        "ps_vlnv": ps.get("VLNV"),
        "ip_instance": accelerator.get("INSTANCE"),
        "ip_vlnv": accelerator.get("VLNV"),
        "clocks": clocks,
        "io_pll_frequency_hz": pll_hz,
        "clock_hz": 100000000,
        "axi_base": base,
        "axi_range": size,
        "hwh_memory_range": address,
    }
...
def render(fields, hwh_sha):
    lines = [
        ...
        "set {ps_instance} [ create_bd_cell -type ip -vlnv {ps_vlnv} {ps_instance} ]".format(
            **fields
        ),
        "set {ip_instance} [ create_bd_cell -type ip -vlnv {ip_vlnv} {ip_instance} ]".format(
            **fields
        ),
    ...
    for clock in fields["clocks"]:
        i = clock["index"]
        lines.append(
            "  CONFIG.PCW_FCLK{}_PERIPHERAL_DIVISOR0 {{{}}} \\".format(
                i, clock["divisor0"]
            )
        )
        lines.append(
            "  CONFIG.PCW_FCLK{}_PERIPHERAL_DIVISOR1 {{{}}} \\".format(
                i, clock["divisor1"]
            )
        )
        lines.append(
            "  CONFIG.PCW_FPGA_FCLK{}_ENABLE {{{}}} \\".format(i, clock["enabled"])
        )
    lines.extend(
        [
            "] $" + fields["ps_instance"],
            (
                ...
                "[get_bd_addr_segs {ip_instance}/S_AXI/reg0] SEG_maxcut26"
            ).format(**fields),
            "",
        ]
    )
    ...
    require(
        family_match is not None and family_match.group(0) == "xc7z",
