...
def check():
    ...
    require(
        problem['pbits'] == 26 and problem['auxiliary_variables'] == 0,
        'Invalid variable count',
    )
    require(
        len(problem['trial_seeds']) == 1000
        and len(set(problem['trial_seeds'])) == 1000,
        'Invalid paired seeds',
    )
...
def load():
    ...
    require(
        io.read(0) == 0x584F3236 and io.read(4) == 0x30000,
        ...
        with (out / 'trials.jsonl').open('x') as log:
            for trial in range(args.trials):
                ...
                for policy in modes:
                    ...
                    require(
                        row['updates'] <= 26624
                        and row['fresh'] + row['reuse'] == row['updates'],
                        'Flip counts',
                    )
                    require(
                        row['initial_draws'] == 26
                        and row['rng_words'] == 26 + row['fresh'] + row['refills'],
                        'Source counts',
                    )
                    require(
                        row['rng_steps'] == 32 * row['rng_words'], 'LFSR step count'
                    )
                    if policy == 0:
                        require(
                            row['reuse'] == 0 and row['fresh'] == row['updates'],
                            'IID counts',
                        )
                    unsat = sum(
                        not any(
                            ((row['final_bits'] >> (abs(v) - 1)) & 1) == (v > 0)
                            for v in c
                        )
                        for c in problem['clauses']
                    )
                    ...
                    if 'native_problem' in problem:
                        violated = sum(
                            (
                                sum(
                                    (row['final_bits'] >> i) & 1 for i in c['variables']
                                )
                                % 2
                            )
                            != c['parity']
                            for c in problem['native_problem']['native_clauses']
                        )
