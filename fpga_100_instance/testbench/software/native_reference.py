...
intentional algorithm input change is the specified shared serial LFSR random source,
    ...
    for line in Path(path).read_text().splitlines():
        ...
        for v in map(int, line.split()):
            if v:
                ...
                pending = []
    assert 1 <= n <= 32 and len(clauses) == declared and not pending
...
class RandomSource:
    def __init__(self, seed):
        self.state = seed & MASK32 or 0xA5C36EFE
        self.calls = 0
...
class Reference:
    def __init__(self, clauses, seed, mode, age_limit=254, *, n):
        ...
        self.true = [
            sum(self.bits[abs(lit) - 1] == (lit > 0) for lit in c) for c in clauses
        ]
    ...
    def event(self, kind):
        states = [
            sum(self.bits[32 * k + j] << j for j in range(min(32, self.n - 32 * k)))
            for k in range((self.n + 31) // 32)
        ]
        return [
            kind,
            *states,
    ...
    def step(self):
        ...
        if self.mode == "baseline" or self.ages[selected] >= self.age_limit:
            if self.mode == "epix":
                key = (
                    "first_visit_refreshes"
                    if self.ages[selected] == 255
                    else "age_refreshes"
                )
                self.stats[key] += 1
            ...
            self.reuse += 1
        before = self.raw
        chosen, value, span = conditioned(
            self.values[selected], self.ranges[selected], weight, self.word, self.stats
        )
    ...
    def verify_assignment(self):
        ...
        direct = [
            sum(self.bits[abs(lit) - 1] == (lit > 0) for lit in c) for c in self.clauses
        ]
...
def state_hash(event, n):
    ...
    return hashlib.sha256(
        struct.pack("<" + "I" * words, *event[1 : 1 + words])
    ).hexdigest()
