...
The implementation is self-contained except for NumPy so the same source can
run locally, on compute_host, and on the compute cluster.  All stochastic mechanisms use
...
def exact_maxcut_dp(inst: KingsInstance) -> int:
    """Independent exact value DP for sides up to 20."""
    side = inst.side
    if side > 20:
        raise ValueError("sliding-window DP is intentionally capped at side 20")
    window = side + 1
    n_states = 1 << window
    weights = {
        (max(int(u), int(v)), min(int(u), int(v))): int(w)
        for u, v, w in zip(inst.edge_u, inst.edge_v, inst.edge_w)
    }
    states = np.arange(n_states, dtype=np.int64)
    value = np.zeros(n_states, dtype=np.int32)
    half = n_states >> 1
    for node in range(inst.n):
        contrib0 = np.zeros(n_states, dtype=np.int16)
        contrib1 = np.zeros(n_states, dtype=np.int16)
        for distance in (1, side - 1, side, side + 1):
            prior = node - distance
            if prior < 0:
                continue
            weight = weights.get((node, prior))
            if weight is None:
                continue
            bit = (states >> (distance - 1)) & 1
            contrib0 += weight * bit
            contrib1 += weight * (1 - bit)
        next_value = np.empty(n_states, dtype=np.int32)
        next_value[0::2] = np.maximum(
            value[:half] + contrib0[:half], value[half:] + contrib0[half:]
        )
        next_value[1::2] = np.maximum(
            value[:half] + contrib1[:half], value[half:] + contrib1[half:]
        )
        value = next_value
...
def attempt_icm(
    ...
    """Attempt one low-temperature ICM; return accepted, size, words, reason."""
...
def update_color(
    ...
    return visits, int(np.sum(fire)), fresh
...
def run_apt_icm(
    ...
    if method not in {"iid_apt_icm", "epic_apt_icm"}:
    ...
    if target_kind not in {"exact_dp", "exact_cplex", "validation"}:
    ...
    for sweep_index in range(max_sweeps):
        ...
        for color in range(4):
            visits, fired, fresh = update_color(
                inst,
                bank,
                betas,
                color,
...
def iid_fixed_beta_sweep(
    ...
    for color in range(4):
        group = np.flatnonzero(inst.colors == color)
        spins = 2 * state.astype(np.int16) - 1
        spins_pad = np.append(spins, 0)
        local = -np.sum(
            inst.neigh_w[group] * spins_pad[inst.neigh_idx[group]],
            axis=1,
            dtype=np.int32,
        )
        field_input = np.clip(
            np.rint(FIELD_SCALE * beta * local), -FIELD_LIMIT, FIELD_LIMIT
        ).astype(np.int16)
        threshold = LUT_TABLE[np.abs(field_input)]
        words = fresh_u32(rng, len(group))
        state[group] = np.where(
            field_input >= 0, threshold > words, words > threshold
        ).astype(np.int8)


def ladder_from_energy_sigma(
    beta_grid: np.ndarray, sigma: np.ndarray, temperatures: int
) -> np.ndarray:
    smooth = sigma.copy()
    if len(sigma) >= 3:
        smooth[1:-1] = (
            0.25 * sigma[:-2] + 0.50 * sigma[1:-1] + 0.25 * sigma[2:]
        )
    increments = 0.5 * (smooth[:-1] + smooth[1:]) * np.diff(beta_grid)
    cumulative = np.concatenate(([0.0], np.cumsum(increments)))
    if cumulative[-1] <= 0:
        ladder = np.geomspace(beta_grid[0], beta_grid[-1], temperatures)
    else:
        ladder = np.interp(
            np.linspace(0.0, cumulative[-1], temperatures), cumulative, beta_grid
        )
    ladder[0], ladder[-1] = beta_grid[0], beta_grid[-1]
    return ladder


def calibrate_iid_ladder(
    inst: KingsInstance,
    temperatures: int,
    beta_min: float,
    beta_max: float,
    ...
    """IID-only thermodynamic-length preprocessing for an adaptive ladder."""
    rng = stream_rng(seed, 91)
    beta_grid = np.linspace(beta_min, beta_max, grid_points)
    sigma = np.empty(grid_points, dtype=np.float64)
    means = np.empty(grid_points, dtype=np.float64)
    for index, beta in enumerate(beta_grid):
        state, _ = initialize_bits(rng, (inst.n,))
        samples = []
        for sweep in range(burn_sweeps + sample_sweeps):
            iid_fixed_beta_sweep(inst, state, float(beta), rng)
            if sweep >= burn_sweeps:
                cut = cut_bits_direct(inst, state)
                samples.append(int(energy_from_cut(inst, cut)))
        means[index] = float(np.mean(samples))
        sigma[index] = max(float(np.std(samples, ddof=1)), 1e-9)
    betas = ladder_from_energy_sigma(beta_grid, sigma, temperatures)
    return {
        "schema": "iid_thermodynamic_length_ladder_v1",
        "instance": inst.name,
        "temperatures": temperatures,
        "beta_grid": beta_grid.tolist(),
        "energy_mean": means.tolist(),
        "energy_sigma": sigma.tolist(),
        "betas": betas.tolist(),
        "beta_ladder_id": ladder_id(betas),
        "seed": int(seed),
        "burn_sweeps": burn_sweeps,
        "sample_sweeps": sample_sweeps,
        "local_node_updates": int(
            grid_points * (burn_sweeps + sample_sweeps) * inst.n
        ),
        "instance_digest": instance_digest(inst),
    }
