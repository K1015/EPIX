...
module corrected_solver #(
    ...
    localparam integer NODE_COUNT=512, GRAPH_WIDTH=16;
    localparam integer REPLICAS=48, TEMPERATURES=24, COLOR_NODES=128;
    localparam integer REUSE_LIMIT=4, REMAINING_BITS=3;
    localparam integer ICM_FIRST=18, ICM_LAST=23;
    localparam integer ICM_INTERVAL=4, ICM_MAX_DISAGREEMENT=256;
    localparam integer POP_COUNT_BITS=10, FRESH_COUNT_BITS=8;
    localparam logic signed [11:0] SCORE_INITIAL=-2048;
    localparam logic signed [11:0] EXACT_TARGET=500;
    ...
    wire [511:0] spins[0:REPLICAS-1];
    logic signed [11:0] cuts[0:REPLICAS-1];
    wire [511:0] current=spins[replica];
    ...
    wire [511:0] disagreement=spins[icm_temp]^spins[icm_temp+TEMPERATURES];
    logic [511:0] cluster;
    wire [511:0] expanded=expand(cluster)&disagreement;
    wire [511:0] invalidated=expand(cluster);
    logic [POP_COUNT_BITS-1:0] disagreement_count;
    logic [511:0] seed_mask;
    ...
    localparam integer RNG_BANKS=16;
    wire [31:0] local_rng_word[0:511];
    ...
    for(genvar i=0;i<NODE_COUNT;i=i+1) begin: lane
        ...
        for(genvar r=0;r<REPLICAS;r=r+1) begin: storage
            ...
            always_ff @(posedge bank_clock[r][NODE_COLOR]) begin
                if(rst) begin spin_reg<=0;remaining_reg<=0;saved_field_reg<=0;end
                else begin
                    if(init_we && phase==IDLE && init_bank==r && init_word==i/32)
                        spin_reg<=data_in[i%32];
                    if(start && phase==IDLE) begin remaining_reg<=0;saved_field_reg<=0;end
                    if(local_commit && selected && replica==r) begin
                        if(refresh) begin
                            spin_reg<=sample;remaining_reg<=REMAINING_BITS'(REUSE_LIMIT); saved_field_reg<=encoded_field;
                        end else remaining_reg<=remaining_reg-1'b1;
                    end
                    if(phase==FLIP && (icm_temp==r || icm_temp+TEMPERATURES==r)) begin
                        spin_reg<=spins[r][i]^cluster[i];
                        if(invalidated[i]) remaining_reg<=0;
                    end
                    if(phase==SWAP && accept_swap) begin
                        if(swap_a==r) begin spin_reg<=spins[swap_b][i];remaining_reg<=0;end
                        if(swap_b==r) begin spin_reg<=spins[swap_a][i];remaining_reg<=0;end
                    end
                end
            end
        end
