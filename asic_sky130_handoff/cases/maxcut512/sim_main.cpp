...
#include <string>
#include <algorithm>
using Words=std::array<uint32_t,MAXCUT_REPLICAS*(MAXCUT_N/32)>;
...
int main(int argc,char**argv){
 try {
    ...
    auto state=[&](){Words a;for(int r=0;r<MAXCUT_REPLICAS;r++){dut->read_bank=r;dut->eval();for(int j=0;j<(MAXCUT_N/32);j++)a[r*(MAXCUT_N/32)+j]=
#if MAXCUT_N == 32
 dut->state_read;
#else
 dut->state_read[j];
#endif
}dut->read_bank=0;dut->eval();return a;};
    receipt<<"trial\tsetup_begin\tsearch_begin\tend\tsuccess\tsweeps\tlocal_words\tswap_words\ticm_words\tvisits\tevents\n";
    dut->rst=1;dut->start=0;dut->seed_we=0;dut->init_we=0;dut->read_bank=0;dut->sweep_limit=MAXCUT_SWEEPS;tick();tick();
    for(unsigned k=0;k<first+count;k++){
        unsigned trial=get(input);std::array<uint32_t,MAXCUT_N+2> seeds;for(auto&w:seeds)w=get(input);
        ...
        dut->rst=1;dut->start=0;dut->seed_we=0;dut->init_we=0;dut->read_bank=0;dut->sweep_limit=MAXCUT_SWEEPS;tick();tick();
        dut->rst=0;
        dut->seed_we=1;for(int i=0;i<MAXCUT_N+2;i++){dut->seed_addr=i;dut->data_in=seeds[i];tick();}dut->seed_we=0;
        dut->init_we=1;for(int r=0;r<MAXCUT_REPLICAS;r++)for(int j=0;j<(MAXCUT_N/32);j++){dut->init_bank=r;dut->init_word=j;dut->data_in=initial[r*(MAXCUT_N/32)+j];tick();}dut->init_we=0;
        ...
        while(!dut->done){
            ...
            if(cycles-begin>uint64_t(MAXCUT_SWEEPS)*(MAXCUT_REPLICAS*8+MAXCUT_N*MAXCUT_REPLICAS+100))throw std::runtime_error("timeout");
        }
        uint64_t end=cycles;
        int best=(dut->best_cut&(1u<<(MAXCUT_SCORE_BITS-1)))?static_cast<int>(dut->best_cut)-(1u<<MAXCUT_SCORE_BITS):dut->best_cut;
