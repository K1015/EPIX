...
def generated_model_hashes(directory):
    return {p.name: sha(p) for p in sorted(directory.iterdir())
            if p.name.startswith("Vcorrected_solver") and p.suffix in (".cpp", ".h", ".mk")}


def validate_elaboration(directory, expected):
    manifest = directory / "elaboration.json"
    if not manifest.is_file():
        raise RuntimeError("No completed elaboration manifest")
    actual = json.loads(manifest.read_text())
    models = actual.pop("generated_model_sources", None)
    if actual != expected or not models or models != generated_model_hashes(directory):
        raise RuntimeError("Elaboration source, command, tool, or generated model changed")
    if not (directory / "Vcorrected_solver.mk").is_file():
        raise RuntimeError("Elaborated model is missing its Makefile")
...
def validate_rtl(args, mode):
    ...
    files = [*sorted(source.glob("*.sv*")), ROOT / "sim_main.cpp", ROOT / "dimensions.hpp"]
    ...
    command = [tool, "--cc", "--exe",
               "--trace", "--trace-depth", "1", "--output-split", "10000", "-Wno-fatal", "--top-module", "corrected_solver",
    ...
    elaboration = directory / "elaboration.json"
    expected_elaboration = dict(sources=hashes, command=command,
        verilator_version=subprocess.check_output([tool, "--version"], text=True).strip())
    if args.reuse_elaboration:
        validate_elaboration(directory, expected_elaboration)
    else:
        run(command, directory / "build.log", args.build_timeout)
        elaboration.write_text(json.dumps(dict(expected_elaboration, generated_model_sources=generated_model_hashes(directory)), indent=2) + "\n")
    make = ["make"]
...
def main():
    ...
    parser.add_argument("--reuse-elaboration", action="store_true", help="Reuse only a source- and tool-bound completed Verilator elaboration")
    ...
    if not 1 <= args.trials <= 1000 or not 1 <= args.jobs <= 2:
        parser.error("trials must be 1..1000 and jobs must be 1..2")
    args.work_dir = args.work_dir.resolve()
    args.work_dir.mkdir(parents=True, exist_ok=True)
    modes = ("baseline", "epix") if args.mode == "both" else (args.mode,)
    if args.vectors:
        sys.path.insert(0,str(ROOT.parents[1]/"software/maxcut_hardware"))
        from make_vectors import prepare
        prepare(ROOT, args.work_dir / "vectors", args.trials, modes)
    if args.rtl:
        for mode in modes:
