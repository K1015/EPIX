...
module corrected_solver #(
    ...
    input logic [0:0] init_word,
    input logic [31:0] data_in,
    input logic [15:0] sweep_limit,
    output logic done, success,
    output logic [15:0] sweeps,
    output logic signed [7:0] best_cut,
    output logic [31:0] state_read, witness,
    ...
    localparam integer NODE_COUNT=32, GRAPH_WIDTH=4;
    localparam integer REPLICAS=24, TEMPERATURES=12, COLOR_NODES=8;
    localparam integer REUSE_LIMIT=2, REMAINING_BITS=2;
    localparam integer ICM_FIRST=8, ICM_LAST=11;
    localparam integer ICM_INTERVAL=4, ICM_MAX_DISAGREEMENT=16;
    localparam integer POP_COUNT_BITS=6, FRESH_COUNT_BITS=4;
    localparam logic signed [7:0] SCORE_INITIAL=-128;
    localparam logic signed [7:0] EXACT_TARGET=29;
    ...
    logic [31:0] spins[0:REPLICAS-1];
    logic signed [7:0] cuts[0:REPLICAS-1];
    wire [31:0] current=spins[replica];
    wire signed [7:0] current_cut=score(current);
    ...
    wire signed [8:0] cut_difference=$signed({cuts[swap_b][7],cuts[swap_b]})-$signed({cuts[swap_a][7],cuts[swap_a]});
    ...
    wire accept_swap=(cut_difference<=0) || (swap_next<swap_threshold(swap_left,cut_difference[7:0]));
    wire [31:0] disagreement=spins[icm_temp]^spins[icm_temp+TEMPERATURES];
    logic [31:0] cluster;
    wire [31:0] expanded=expand(cluster)&disagreement;
    wire [31:0] invalidated=expand(cluster);
    logic [POP_COUNT_BITS-1:0] disagreement_count;
    logic [31:0] seed_mask;
    ...
    localparam integer RNG_BANKS=1;
    wire [31:0] local_rng_word[0:31];
