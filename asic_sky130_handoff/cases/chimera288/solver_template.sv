...
`endif
// 288-bit native colored Gibbs search. All decisions and scores are computed here.
module chimera_solver #(
    ...
    localparam integer RNG_BANKS=288/32;
    wire [287:0] state_bits;
    wire [31:0] rng_words [0:287];
    ...
    wire [287:0] refresh_mask,fire_mask;
    logic [15:0] fresh_count,fire_count;
    always_comb begin
        fresh_count=0;
        fire_count=0;
        for(integer i=0;i<288;i=i+1) begin
    ...
    for(genvar i=0;i<288;i=i+1) begin: lane
        localparam bit NODE_COLOR=1'(((i/48)+(i/8)%6+(i/4)%2)%2);
        localparam logic [3:0] NODE_WORD=4'(i/32);
        logic spin;
    ...
    always_ff @(posedge clk) begin
        if(rst) begin
            ...
            INITIAL: begin
                ...
                if(current_cut==150) begin success<=1;first_exact_sweep<=0;witness<=state_bits;end
                phase<=UPDATE;
            end
            UPDATE: if(commit) begin
                local_words<=local_words+{16'b0,fresh_count};
                node_visits<=node_visits+32'd144;
            ...
            SCORE: begin
                ...
                if(current_cut==150 && !success) begin success<=1;first_exact_sweep<={16'b0,sweeps}+32'd1;witness<=state_bits;end
                if(!color) begin color<=1;phase<=UPDATE;end
                else begin
                    color<=0;sweeps<=sweeps+1;
                    if(sweep_in_beta==10'(SWEEPS_PER_BETA-1)) begin
                        sweep_in_beta<=0;
                        if(beta_step==4'(BETA_COUNT)) begin final_success<=(current_cut==150);phase<=FINISH;end
