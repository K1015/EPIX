...
EDGE_U=np.asarray([u for u,v,w in EDGES]); EDGE_V=np.asarray([v for u,v,w in EDGES]); EDGE_W=np.asarray([w for u,v,w in EDGES])
...
def initial(trial):
    seed = int(json.loads((ROOT/'metadata.json').read_text())['replay_seed_base']) + trial
    rng = random.Random(seed)
    seeds = np.asarray([rng.getrandbits(32) for _ in range(N)], dtype=np.uint32)
    seeds[seeds == 0xffffffff] = 0 # XNOR all-ones lock-up is excluded.
    init_words = np.asarray([rng.getrandbits(32) for _ in range(N)], dtype=np.uint64)
...
def run(trial, mode, sweeps_per_beta=937, beta_count=14):
    ...
    rng = seeds.copy()
    saved = np.zeros(N, dtype=np.int16)
    age = np.zeros(N, dtype=np.uint16)
    valid = np.zeros(N, dtype=bool)
    best = score(state)
    first = 0 if best == TARGET else 0xffffffff
    witness = state.copy() if best == TARGET else np.zeros(N, dtype=np.int8)
    events = []
    batches = []
    policy_events = []
    ...
    for beta_step in range(1, beta_count + 1):
        for _ in range(sweeps_per_beta):
            ...
            for color, group in enumerate(GROUPS):
                ...
                address = np.minimum((4*np.abs(numerator)+NORMALIZER//2)//NORMALIZER, 127)
                ...
                if current == TARGET and first == 0xffffffff:
                    first = sweep
                    witness = state.copy()
                events.append((1 + color, state.copy(), current, best, words, visits, fired, first))
    result = dict(trial=trial, seed=seed, mode=mode, success=int(first != 0xffffffff), final_success=int(score(state) == TARGET),
...
def validate_native(record, sweeps_per_beta=937, beta_count=14):
    ...
    import apt_icm
    instance = instances._build_instance(GRAPH['name'], SIDE, {(u, v): w for u, v, w in EDGES})
    assert np.array_equal(instance.colors, COLORS)
    assert np.array_equal(LUT, apt_icm.LUT_TABLE)
    for step in range(1, 15):
        for raw in range(-128,128):
            integer_address = min((4 * abs(step*raw) + NORMALIZER//2)//NORMALIZER, 127)
            native_address = abs(int(np.clip(np.rint(8*(step/2)*raw/NORMALIZER), -127, 127)))
    ...
    def observe(state, current_cut, sweep, color, local_words, visits, fired_cycles, refresh, bfield, field_input, age_group):
        ...
        assert np.allclose(numerator/FIELD_DENOMINATOR, bfield, rtol=0, atol=1e-14), ('native unrounded field', index)
        expected_input = np.minimum((4*np.abs(numerator)+NORMALIZER//2)//NORMALIZER, 127)*np.where(numerator>=0,1,-1)
    ...
    out = namespace['run_colored_anneal_dual'](inst=instance, target=TARGET, trial=record['result']['trial'], seed=record['result']['seed'],
