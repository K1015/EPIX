...
import struct
import time
from reference import ROOT, NATIVE, N, GROUPS, run, validate_native
FIELDS=['success','final_success','sweeps','best_cut','final_cut','first_exact_sweep','local_words','visits','fired_cycles']
def sha(path): return hashlib.sha256(path.read_bytes()).hexdigest()
def packed(state): return [sum(int(state[32*k+j])<<j for j in range(32)) for k in range(N//32)]
...
def search_cycles(record):
    ...
    return cycles
def sources():
    names=['reference.py','make_vectors.py','graph.json','software_spec.json','certificate.json','metadata.json']
