...
`define EPIX_MODE 0
`endif
// 512-bit native colored Gibbs search. All decisions and scores are computed here.
module chimera_solver #(
    ...
    localparam integer RNG_BANKS=512/32;
    wire [511:0] state_bits;
    wire [31:0] rng_words [0:511];
    ...
    wire [511:0] refresh_mask,fire_mask;
    ...
    for(genvar i=0;i<512;i=i+1) begin: lane
        localparam bit NODE_COLOR=1'(((i/64)+(i/8)%8+(i/4)%2)%2);
