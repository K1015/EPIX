...
def validate_rtl(args, mode):
    ...
    command = [tool, "--cc", "--exe",
               "--trace", "-Wno-fatal", "--top-module", "chimera_solver",
               "--prefix", "Vchimera_solver", "--Mdir", str(directory),
    ...
    build_command = [*make, "-C", str(directory), "-f", "Vchimera_solver.mk",
                     "-j", str(args.jobs)]
    run(build_command, directory / "compile.log", args.build_timeout)
    receipt_path = directory / "trials.tsv"
    run([str(directory / "Vchimera_solver"), str(vectors / (mode + ".bin")),
    ...
    for row,outcome in zip(rows,vector_receipt['outcomes']):
        if int(row['end'])-int(row['search_begin']) != outcome['rng32_search_cycles']:
            raise RuntimeError("RNG service cycles differ from the reference request counts")
    ...
    receipt = dict(schema_version=1, passed=True, stage="rtl", mode=mode,
                   ...
                   checks="Every color state, scores, counters and final witness over all 13118 sweeps; state/counters held during service; exact bank-service cycle count",
