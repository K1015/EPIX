...
#include <string>
#include <algorithm>
using State=std::array<uint32_t,25>;
...
int main(int argc,char**argv){try{
    ...
    auto state=[&](){State x;for(int j=0;j<25;j++)x[j]=dut->state_read[j];return x;};
    auto witness=[&](){State x;for(int j=0;j<25;j++)x[j]=dut->witness[j];return x;};
    ...
    for(unsigned k=0;k<first+count;k++){
        unsigned trial=get(input);auto seeds=get_array<800>(input);auto initial=get_array<25>(input);auto expected=get_array<9>(input);
        auto final=get_array<25>(input);auto expected_witness=get_array<25>(input);
        unsigned n=get(input);std::vector<Event>events(n);
        for(auto&e:events){e.kind=get(input);e.state=get_array<25>(input);e.values=get_array<6>(input);}
        if(k<first)continue;
        uint64_t setup=cycles;
        dut->rst=1;dut->start=0;dut->seed_we=0;dut->init_we=0;tick();tick();dut->rst=0;
        dut->seed_we=1;for(int i=0;i<800;i++){dut->seed_addr=i;dut->data_in=seeds[i];tick();}dut->seed_we=0;
        dut->init_we=1;for(int j=0;j<25;j++){dut->init_word=j;dut->data_in=initial[j];tick();}dut->init_we=0;
