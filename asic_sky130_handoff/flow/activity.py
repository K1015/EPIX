"""Stream traces into exact cell-pin activity counts.

Adapted for this package: explicit Liberty time units for activity density.
"""
...
def write_window(directory, window, targets, counts, index, duration, period, ns_per_time_unit):
    """Apply and retain the exact activity expected for each counted target."""
    ...
    for i, (kind, name, _, _) in enumerate(targets):
        ...
        duty = high / duration
        commands.append(f"epix_annotate {kind} {{{name}}} {density:.16g} {duty:.16g} {density_hz:.16g}")
    ...
    return dict(cell_pins=pins, input_ports=inputs, annotated_targets=pins + inputs,
                root_clock=dict(density_hz=2e9 / period, duty=0.5))


def validate_readback(log, expected):
    """Require complete numerical readback after every power report."""
    rows = {}
    for line in log.splitlines():
        ...
        if (checked, pins, inputs, errors) != (want["annotated_targets"], want["cell_pins"], want["input_ports"], 0):
            raise ValueError("Incomplete or failed activity readback")
        root = want["root_clock"]
        if not (origin == "clock" and math.isfinite(density) and math.isfinite(duty)
        ...
        rows[key] = dict(checked=checked, cell_pins=pins, input_ports=inputs, errors=errors,
                         root_clock=dict(density_hz=density, duty=duty, origin=origin))
    if set(rows) != {(window, phase) for window in expected for phase in ("total", "rng")}:
        raise ValueError("Missing activity readback phase")
    return dict(passed=True, expected=expected,
                windows={w: {p: rows[w, p] for p in ("total", "rng")} for w in expected},
                total_annotation_checks=sum(r["checked"] for r in rows.values()),
                required_annotation_origin="user", root_clock_checked_separately=True,
                density_relative_tolerance=1e-5, density_absolute_tolerance_hz=1e-12,
...
def prepare(work, mode, period, top, ns_per_time_unit=1.0):
    ...
    for k, window in enumerate(["search", "with_setup"]):
        coverage = write_window(directory, window, targets, counts, k, durations[k], period, ns_per_time_unit)
        summary = dict(
