"""Insert balanced, noninverting clock buffers into a mapped Yosys JSON design.

Adapted from hardware/maxcut128_corrected_compute_host/repair_clock_fanout.py and
repair_electrical_outputs.py. Technology cells, pins and capacitance units are
parameters here; the original connectivity and characterized-load guards remain.

This is a structural netlist transform; it does not change RTL or run synthesis.
Capacitance checks here cover characterized cell-pin loads. Final STA must also
check the actual net model, clock propagation, transition limits, and timing.
"""
...
def _body(text, opening):
    """Return one braced Liberty body, ignoring braces inside quoted strings."""
...
def _outer(text):
    # SKY130 pin attributes are outside nested timing/power groups.
    lines, depth = [], 0
...
class PinLibrary:
    ...
    def cap(self, cell_type, pin_name):
        ...
        values = [attrs[k] for k in ("capacitance", "rise_capacitance", "fall_capacitance") if k in attrs]
        if not values:
            raise ValueError(f"Missing characterized input capacitance: {cell_type}/{pin_name}")
        return max(values)
...
def repair_clock_design(design, *, library, icg, buffer_config, max_fanout=16,
                        module_name="corrected_solver", expected_gates=None):
    """Return a new JSON object and a structural/cell-load audit dictionary."""
    ...
    numeric_bits = [bit for cell in cells.values() for bits in cell["connections"].values()
                    for bit in bits if isinstance(bit, int)]
    numeric_bits += [bit for net in module["netnames"].values() for bit in net["bits"] if isinstance(bit, int)]
    numeric_bits += [bit for port in module["ports"].values() for bit in port["bits"] if isinstance(bit, int)]
    ...
    def buffer(source):
        ...
        bit = next_bit
        next_bit += 1
        cells[name] = dict(hide_name=0, type=buffer_type, parameters={}, attributes={},
    ...
    for name in gates:
        ...
        gate_audits.append(dict(gate=name, original_fanout=len(sinks),
                               original_pin_load_lib_units=original_cap,
                               inserted_buffers=len(inserted) - first,
                               min_buffer_depth=min(depths), max_buffer_depth=max(depths)))

    # Every original input outside the designated clock trees must be unchanged.
    ...
    for name in gates + inserted:
        ...
        load_audits.append(dict(instance=name, fanout=len(sinks),
                               pin_load_lib_units=cap, max_capacitance_lib_units=limit))
    final_outputs = collections.Counter(bit for cell in cells.values()
        for port, bits in cell["connections"].items() if cell["port_directions"][port] == "output"
        for bit in bits if isinstance(bit, int))
    for bit in parents:
        assert final_outputs[bit] == 1
    audit = dict(passed=True, module=module_name, clock_gates=len(gates),
        max_fanout=max_fanout, buffer_type=buffer_type, inserted_buffers=len(inserted),
        original_clock_sink_pins=len(original_wiring),
        original_max_fanout=max(row["original_fanout"] for row in gate_audits),
        original_max_pin_load_lib_units=max(row["original_pin_load_lib_units"] for row in gate_audits),
        repaired_max_fanout=max(row["fanout"] for row in load_audits),
        repaired_max_pin_load_lib_units=max(row["pin_load_lib_units"] for row in load_audits),
        repaired_max_capacitance_ratio=max(row["pin_load_lib_units"] / row["max_capacitance_lib_units"] for row in load_audits),
        ...
        original_sink_roots_preserved=True, unaffected_input_connections_preserved=True,
        gates=gate_audits, repaired_driver_loads=load_audits)
    return design, audit
...
def violation_pins(report):
    ...
    if len(matches) != report.count("(VIOLATED)"):
        raise ValueError("Unrecognized electrical violation row; refusing partial parsing")
    if re.search(r"Warning|Error", report, re.IGNORECASE):
        raise ValueError("Electrical report contains a warning or error")
    return [match.group(1) for match in matches]


def repair_electrical_design(design, electrical_report, *, library, buffer_config,
                             module_name="corrected_solver", output_load_lib_units=0.01):
    """Insert capacitance-bounded noninverting trees and audit every repaired load."""
    buffer_type = buffer_config["cell"]
    buffer_in, buffer_out = buffer_config["input_pin"], buffer_config["output_pin"]
    ...
    if any(name.startswith("electrical_buffer_") for name in cells):
        raise ValueError("Electrical output repair already present")
    pinlib = PinLibrary(library)
    buffer_input_cap = pinlib.cap(buffer_type, buffer_in)
    buffer_limit = pinlib.pin(buffer_type, buffer_out)["max_capacitance"]
    ...
    for label in reported:
        ...
        if len(found) != 1:
            raise ValueError(f"Reported pin {label} has {len(found)} cell output drivers")
        selected.setdefault(found[0], dict(bit=bit, reported_pins=[]))["reported_pins"].append(label)
    ...
    for name, port in ports.items():
        for index, bit in enumerate(port["bits"]):
            if port["direction"] == "output":
                ...
                raise ValueError(f"Selected output net also drives an input/inout port: {name}")
    replacements, audit_rows, inserted = {}, [], []
    before_connections = {name: copy.deepcopy(cell["connections"]) for name, cell in cells.items()}
    ...
    if not all(math.isfinite(x) and x > 0 for x in (buffer_input_cap, buffer_limit)):
        raise ValueError("Buffer input and maximum output capacitances must be positive and finite")
    ...
    def buffer(source):
        nonlocal next_bit
        name = f"electrical_buffer_{len(inserted):06d}"
        net_name = name + "_out"
    ...
    def connect(source, sink):
        ...
        if kind == "cell":
            ...
            # Yosys output declarations and same-name nets must agree.
            if name not in nets:
                nets[name] = dict(hide_name=0, bits=list(ports[name]["bits"]), attributes={})
            else:
    ...
    def distribute(source, pending):
        ...
        if fits(load, buffer_limit):
            ...
            return
        if max_branches < 2:
            raise ValueError(f"{buffer_type} cannot drive two buffer inputs; a load-reducing tree is impossible")
        # Balance loads by characterized capacitance. Every
        # branching output drives at most max_branches identical buffer inputs.
        groups = min(max_branches, len(pending), max(2, math.ceil(load / buffer_limit)))
        if groups < 2:
            raise ValueError(f"One {buffer_type} cannot drive an indivisible sink load of {load}")
    ...
    for driver, entry in sorted(selected.items()):
        ...
        if not math.isfinite(source_limit) or source_limit <= 0:
            raise ValueError(f"Missing positive source maximum capacitance: {name}/{pin}")
        ...
        pending.extend(("port", pn, "", pi, output_load_lib_units) for pn, pi in external_sinks)
        if not pending:
        ...
        for sink in pending:
            ...
            if not fits(sink[4], buffer_limit):
                raise ValueError(f"One {buffer_type} cannot drive indivisible sink {sink[:4]} of {sink[4]} library capacitance units")
        ...
        for kind, sn, sp, si, _ in pending:
            actual = cells[sn]["connections"][sp][si] if kind == "cell" else ports[sn]["bits"][si]
            depth = 0
            ...
            depths.append(depth)
        audit_rows.append(dict(driver=f"{name}/{pin}", driver_cell_type=cells[name]["type"],
            reported_pins=sorted(set(entry["reported_pins"])), buffer=root_name,
            inserted_buffers=len(inserted)-first,
            min_buffer_depth=min(depths), max_buffer_depth=max(depths),
            original_fanout=len(cell_sinks), original_output_port_bits=len(external_sinks),
            original_load_lib_units=original_load, source_repaired_load_lib_units=buffer_input_cap,
            source_max_capacitance_lib_units=source_limit, buffer_max_capacitance_lib_units=buffer_limit))
    ...
    final_outputs = collections.Counter(bit for port in ports.values() if port["direction"] == "output"
                                        for bit in port["bits"])
    load_rows = []
    audited = [(name, pin) for name, pin, _ in sorted(selected)] + [(name, buffer_out) for name in inserted]
    for name, pin in audited:
        ...
        load_rows.append(dict(instance=name, output_pin=pin, fanout=len(loads), output_port_bits=final_outputs[bit],
                              load_lib_units=cap, max_capacitance_lib_units=limit))
    ...
    output_drivers = collections.Counter(bit for cell in cells.values()
        for pin, bits in cell["connections"].items() if cell["port_directions"][pin] == "output"
        for bit in bits if isinstance(bit, int))
    assert all(output_drivers[bit] == 1 for bit in replacements), "Repair output has multiple drivers"
    return design, dict(passed=True, reported_violation_rows=len(reported),
        inserted_buffers=len(inserted), buffer_type=buffer_type,
        original_input_and_output_truth_preserved=True,
        unaffected_connections_preserved=True, output_load_lib_units_per_bit=output_load_lib_units,
        max_buffer_branches=max_branches,
        repaired_max_capacitance_ratio=max((row["load_lib_units"]/row["max_capacitance_lib_units"] for row in load_rows), default=0),
        load_basis="Worst characterized cell-pin capacitance plus declared output loads; excludes wires",
        library_sha256=hashlib.sha256(pathlib.Path(library).read_bytes()).hexdigest(),
        repairs=audit_rows, repaired_driver_loads=load_rows)
