...
MASK32 = (1 << 32) - 1
EVENT_FIELDS = (
    "kind", "state0", "state1", "state2", "state3", "flips", "raw_words",
    ...
    for line in Path(path).read_text().splitlines():
        ...
        for v in map(int, line.split()):
            if v:
                ...
                pending = []
    assert n == 128 and len(clauses) == declared and not pending
...
class Reference:
    ...
    def event(self, kind):
        states = [sum(self.bits[32*k+j] << j for j in range(32)) for k in range(4)]
        return [kind, *states, self.flips, self.raw, self.reuse, self.refills,
                128, len(self.false), self.source.state, self.selected,
...
def state_hash(event):
    return hashlib.sha256(struct.pack("<4I", *event[1:5])).hexdigest()
