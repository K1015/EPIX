...
def source_hashes(case):
    files = [*HERE.glob("*.py"), HERE / "sim_main.cpp",
             HERE.parent / "probsat_native/probsat_v20.c",
             HERE.parent / "probsat_native/conditioned.h",
    ...
    for mode in modes:
        ...
        for trial in range(trials):
            ...
            native_events, native_receipt = run(binary, case / "input.cnf", seed,
                                                mode, limit, prefix)
            model = Reference(clauses, seed, mode)
            ...
            for event_index, (actual, expected) in enumerate(zip(events, native_events)):
                if actual != expected:
                    details = [(EVENT_FIELDS[j], a, b) for j, (a, b)
            ...
            outcomes.append(dict(trial=trial, seed=seed, success=solved,
                ...
                initial_state_sha256=state_hash(events[0]),
                final_state_sha256=state_hash(events[-1]), flips=model.flips,
                raw_words=model.raw, reused_visits=model.reuse, refills=model.refills,
                initial_draws=128, events=len(events), num_unsat=len(model.false),
                directed_coverage=model.stats, native_oracle=native_receipt))
        path = directory / (mode + ".bin")
        with path.open("wb") as out:
            out.write(struct.pack("<3I", MAGIC, 1, trials))
            for trial, seed, budget, success, events in records:
                out.write(struct.pack("<5I", trial, seed, budget, success, len(events)))
                for event in events:
                    assert len(event) == len(EVENT_FIELDS)
                    out.write(struct.pack("<"+"I"*len(EVENT_FIELDS), *event))
        ...
        receipt = dict(schema_version=1, passed=True, case=case.name, mode=mode,
            ...
            event_fields=EVENT_FIELDS, native_oracle=build_receipt,
