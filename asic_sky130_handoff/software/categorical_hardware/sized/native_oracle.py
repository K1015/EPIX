...
NATIVE = HERE.parent.parent / "probsat_native"
...
static void hw_emit(int kind) {
 if(!hw_trace)return;
 unsigned words=(numVars+31)/32;uint32_t s[words];memset(s,0,sizeof(s));
 ...
 fprintf(hw_trace,"%u",(unsigned)kind);
 for(unsigned j=0;j<words;j++)fprintf(hw_trace," %u",s[j]);
 fprintf(hw_trace," %u %u %u %u %u %u %u %u %u %u %u %u %u %u %u %u %u %u %u %u\n",
 kind==2?(unsigned)(flip+1):0,
