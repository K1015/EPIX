...
`endif
// 128-bit native colored Gibbs search. All decisions and scores are computed here.
module chimera_solver #(
    ...
    localparam integer RNG_BANKS=128/32;
    wire [127:0] state_bits;
    wire [31:0] rng_words [0:127];
    ...
    wire signed [9:0] current_cut=score(state_bits);
    wire [127:0] refresh_mask,fire_mask;
    logic [7:0] fresh_count,fire_count;
    always_comb begin
        fresh_count=0;
        fire_count=0;
        for(integer i=0;i<128;i=i+1) begin
            fresh_count=fresh_count+{7'b0,refresh_mask[i]};
            fire_count=fire_count+{7'b0,fire_mask[i]};
        end
    end
    ...
    for(genvar i=0;i<128;i=i+1) begin: lane
        localparam bit NODE_COLOR=1'(((i/32)+(i/8)%4+(i/4)%2)%2);
        localparam logic [1:0] NODE_WORD=2'(i/32);
        logic spin;
        logic [31:0] rng_state;
        logic signed [8:0] saved_numerator;
        logic [1:0] remaining;
        wire selected=(phase==UPDATE)&&(color==NODE_COLOR);
        wire signed [5:0] raw_value=raw_field(i,state_bits);
        wire signed [8:0] numerator=$signed(raw_value)*$signed({1'b0,beta_step});
        wire nonnegative=(numerator>=0);
        wire [8:0] magnitude=nonnegative?numerator:-numerator;
        wire [10:0] unbounded_address=({2'b00,magnitude}*4+1)/3;
        wire [7:0] address=(unbounded_address>127)?8'd127:unbounded_address[7:0];
        wire [31:0] threshold=probability(address);
        // Adjacent numerator values differ by 1/6, greater than delta=0.05.
    ...
    always_ff @(posedge clk) begin
        if(rst) begin
            ...
            INITIAL: begin
                ...
                if(current_cut==68) begin success<=1;first_exact_sweep<=0;witness<=state_bits;end
                phase<=UPDATE;
            end
            UPDATE: if(commit) begin
                local_words<=local_words+{24'b0,fresh_count};
                node_visits<=node_visits+64;
                fired_cycles<=fired_cycles+{24'b0,fire_count};
                phase<=SCORE;
            end
            SCORE: begin
                final_cut<=current_cut;
                if(current_cut>best_cut) best_cut<=current_cut;
                if(current_cut==68 && !success) begin success<=1;first_exact_sweep<={16'b0,sweeps}+32'd1;witness<=state_bits;end
                if(!color) begin color<=1;phase<=UPDATE;end
                else begin
                    color<=0;sweeps<=sweeps+1;
                    if(sweep_in_beta==10'(SWEEPS_PER_BETA-1)) begin
                        sweep_in_beta<=0;
                        if(beta_step==4'(BETA_COUNT)) begin final_success<=(current_cut==68);phase<=FINISH;end
