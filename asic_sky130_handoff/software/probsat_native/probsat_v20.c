...
static int v11_enabled=0,v11_age_limit=1,v20_relax=0;
static unsigned long long v20_reasons[6]={0};
static double *v11_cdf;
static uint64_t *v16_value,*v16_range;
static unsigned char *v11_age;
static int *v11_chosen;
static unsigned long long v11_fresh=0,v11_reuse=0,v11_init=0,v16_visits=0,v16_refills=0;
static unsigned long long v11_initial=14695981039346656037ull;
static uint32_t source(void){v11_fresh++;return (uint32_t)rand();}
static int v16_choose(int site,double *weights,double total,int length) {
 assert(length==3 && RAND_MAX==2147483647);v16_visits++;
 double cdf[3],cumulative=0,maxweight=0;
 for(int j=0;j<length;j++){cumulative+=weights[j]/total;cdf[j]=cumulative;if(weights[j]>maxweight)maxweight=weights[j];}
 int reason=0;
 if(!v11_enabled)reason=1;
 else if(v11_age[site]==255)reason=2;
 else if(v11_age[site]>=v11_age_limit)reason=3;
 else if(!v20_relax){
  if(v11_chosen[site]<0 || weights[v11_chosen[site]]<maxweight)reason=4;
  if(!reason)for(int j=0;j<length;j++)if(fabs(cdf[j]-v11_cdf[3*site+j])>=.10){reason=5;break;}
 }
 int refresh=reason!=0;v20_reasons[reason]++;
 if(refresh){v16_value[site]=source();v16_range[site]=1ull<<31;v11_age[site]=0;
  for(int j=0;j<length;j++)v11_cdf[3*site+j]=cdf[j];
 }else{v11_age[site]++;v11_reuse++;}
 unsigned w[3];quantize(weights,w);
 unsigned long long before=v11_fresh;
 int chosen=conditioned(&v16_value[site],&v16_range[site],w,source);
 v16_refills+=v11_fresh-before;
 return chosen;
}
static void v11_report(void){printf("\nV20_POLICY %d %d %d\nV20_REASONS %llu %llu %llu %llu %llu %llu\n",v11_enabled,v11_age_limit,v20_relax,v20_reasons[0],v20_reasons[1],v20_reasons[2],v20_reasons[3],v20_reasons[4],v20_reasons[5]);printf("\nV16_COUNTS %llu %llu %llu %llu %016llx %llu\n",v11_fresh,v11_reuse,v11_init,v16_visits,v11_initial,v16_refills);}
...
static inline void init() {
	...
	for (i = 1; i <= numVars; i++) {
		atom[i] = rand() % 2; v11_init++; v11_initial^=(unsigned char)atom[i]; v11_initial*=1099511628211ull;
		breaks[i] = 0;
	}
...
static inline void pickAndFlipNC() {
	...
	i = v16_choose(rClause,probs,sumProb,i);
	bestVar = abs(clause[rClause][i]);
    v11_chosen[rClause]=i;
...
int main(int argc, char *argv[]) {
  ...
  if(minClauseSize!=3 || maxClauseSize!=3 || maxTries!=1) {
    fprintf(stderr,"v11 supports only 3-SAT and one initialization per run\n");return 2;
  }
  v11_enabled=getenv("V11_EPIX")?atoi(getenv("V11_EPIX")):0;
  v20_relax=getenv("V20_RELAX")?atoi(getenv("V20_RELAX")):0;
  if(v20_relax!=0 && v20_relax!=1)return 2;
  v11_age_limit=getenv("V11_AGE")?atoi(getenv("V11_AGE")):1;
  if((v11_enabled!=0 && v11_enabled!=1) || v11_age_limit<0 || v11_age_limit>254)return 2;
  v16_value=calloc(numClauses+1,sizeof(uint64_t));
  v16_range=calloc(numClauses+1,sizeof(uint64_t));
  v11_cdf=calloc(3*(numClauses+1),sizeof(double));
  v11_age=malloc(numClauses+1);memset(v11_age,255,numClauses+1);
  v11_chosen=calloc(numClauses+1,sizeof(int));
  atexit(v11_report);
	setupSignalHandler();
